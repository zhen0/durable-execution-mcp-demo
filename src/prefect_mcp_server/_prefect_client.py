    ...
    try:
        async with get_client() as client:
            ...
            pending_runs = await client.read_flow_runs(
                flow_run_filter=FlowRunFilter(
                    state=FlowRunFilterState(
                        type=FlowRunFilterStateType(any_=[StateType.PENDING, StateType.SCHEDULED])
                    )
                ),
                limit=50
            )
            ...
            failed_runs = await client.read_flow_runs(
                flow_run_filter=FlowRunFilter(
                    state=FlowRunFilterState(
                        type=FlowRunFilterStateType(any_=[StateType.FAILED, StateType.CRASHED])
                    ),
            ...
            for pool in work_pools:
                work_pool_info: WorkPoolInfo = {
                    "name": pool.name,
                    "type": getattr(pool, 'type', 'unknown'),
                    "is_paused": getattr(pool, 'is_paused', False),
